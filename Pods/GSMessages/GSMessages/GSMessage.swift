...
public class GSMessage: NSObject {
    ...
    public func show() {
        ...
        for animation in animations {
            ...
            case let .slide(type):
                ...
                case (let .distance(d), .top): y = CGFloat(-d)
                case (let .distance(d), .bottom): y = CGFloat(d)
                }
    ...
    @objc fileprivate func handleTapForHide(_: UITapGestureRecognizer) {
        hide(animated: true)
    }

    @objc fileprivate func handleTap(_: UITapGestureRecognizer) {
...
@objc fileprivate extension GSMessage {
    func keyboardWillShow(notification: NSNotification) {
        ...
        if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameBeginUserInfoKey] as? NSValue)?.cgRectValue {
            if messageView.frame.origin.y == 0 && inView.frame.origin.y < 0 {
